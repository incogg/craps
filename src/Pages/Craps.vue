...
import HornScenario from "@/Craps/HornScenario";
import HardsScenario from "@/Craps/HardsScenario";
import { useDifficulty } from "@/Common/useDifficulty";

const SCENARIOS = [HornScenario, HardsScenario];
...
const d1     = computed(() => round.value?.d1 ?? 1);
const d2     = computed(() => round.value?.d2 ?? 1);
...
<template>
    <div class="page">
        <nav class="top-nav">
            ...
            <DiffDock />
            <span class="nav-game">{{ currentScenario?.label ?? 'Craps' }}</span>
        </nav>
